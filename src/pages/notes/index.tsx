...
import { getNotes, deleteNotesById } from 'apiSdk/notes';
import { NotesInterface } from 'interfaces/notes';

type ColumnType = ColumnDef<NotesInterface, unknown>;
...
export function NotesListPage(props: NotesListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<NotesInterface>>(
  ...
  const handleView = (row: NotesInterface) => {
    if (hasAccess('notes', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('notes', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('notes', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="notes" />
        </Flex>

        {hasAccess('notes', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
